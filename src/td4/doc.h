 ...
 * We define a PosixThread class that embeds a POSIX thread management. The two constructors allow to instanciate an object from scratch or from an existing thread.
 ...
 * It uses the same mechanism as in the Timer class to be able to pass a static method pointer to the thread configuration POSIX tructure, that calls the virtual method `run` implemented by child classes.
 * Methods enables the time measurement of the thread execution time.
 *
 ...
 * Two other subclasses Lock and TryLock inherit from Monitor. At instanciation they lock or try to lock with a timeout a Mutex, and they unlock it within their destructor.
 *
 * An example of use is given  in `main.cpp`. We create a pool of workers that inherit from Thread and increments a shared value protected by a Mutex.
 * In the end, the variable value has a correct value 100% of the time : the mutex protected it from race conditions.
 *
 ...
 * It exposes a `give` and `take` methods that increment or decrement this counter.
 * If it reached `0` the `take` method will be blocking until a token is added to it with a call to `give`.
 * An overload of `take` with a timeout exists. It returns a bool corresponsing to `true` if it achieved to get a token in time and `false` else.
 ...
 *
 * We create a pool of each of these objects, start them in their own threads (they inherit from Thread) and check that after the all ended, the consumers consumed all the tokens.
 * In order to do that, we try to take from the Semaphore with a timeout and check it returns `false`.
 *
 ...
 *
 * We tested it in `main.cpp` with two type of workers: Consumer that pop values from a FIFO of int, Producer that push values to it.
